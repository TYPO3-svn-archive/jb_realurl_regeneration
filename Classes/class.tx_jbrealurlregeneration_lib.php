...
class tx_jbrealurlregeneration_lib extends tslib_pibase {
	 ...
	 * @param integer olderThan in hour
	 * @return void
	 */
	
	function delete_tables($olderThan=0){
		
		if($olderThan > 0){
			$older = intval(time() - ($olderThan*3600));
		}else{
			$older  = 0;
		}
		$older =($older<0)?0:$older;
				
		
			
		try{
			if($older==0){
				$GLOBALS['TYPO3_DB']->sql_query('TRUNCATE `tx_realurl_pathcache`;');
				$GLOBALS['TYPO3_DB']->sql_query('TRUNCATE `tx_realurl_chashcache`;');
				$GLOBALS['TYPO3_DB']->sql_query('TRUNCATE `tx_realurl_urldecodecache`;');
				$GLOBALS['TYPO3_DB']->sql_query('TRUNCATE `tx_realurl_urlencodecache`;');
			}else{
				$GLOBALS['TYPO3_DB']->exec_DELETEquery('tx_realurl_pathcache','expire<'.$older);		
				$GLOBALS['TYPO3_DB']->sql_query('TRUNCATE `tx_realurl_chashcache`;');	
				$GLOBALS['TYPO3_DB']->exec_DELETEquery('tx_realurl_urldecodecache','tstamp<'.$older);		
				$GLOBALS['TYPO3_DB']->exec_DELETEquery('tx_realurl_urlencodecache','tstamp<'.$older);		
			}
			

			// delete news in tx_realurl_uniqalias ['deleteUniqAliasTables']
			if($this->conf['extConf']['deleteUniqAliasTables']){
				...
				foreach($tables as $t){
					...
					if($older==0){
						...
						$GLOBALS['TYPO3_DB']->exec_DELETEquery('tx_realurl_uniqalias','tablename='.$tc.' AND tstamp<'.$older);
					}
				}	
			}elseif($this->conf['extConf']['deleteUniqAlias']){
				if($older==0){
					$GLOBALS['TYPO3_DB']->sql_query('TRUNCATE `tx_realurl_uniqalias`;');
				}else{
					$GLOBALS['TYPO3_DB']->exec_DELETEquery('tx_realurl_uniqalias','tstamp<'.$older);		
				}
			}
	...
	/**
	 * function to deleteCacheTables
	 * param array of string cache tables
	 * param integer olderThan in hour
	 */

	function deleteCacheTables($tables,$olderThan=0) {
		...
		if (count($tables) > 0) {
				foreach($tables as $table) {
						$GLOBALS['TYPO3_DB']->exec_DELETEquery($table,'tstamp<'.$older);	
						$GLOBALS['TYPO3_DB']->sql_query('OPTIMIZE TABLE `' . mysql_real_escape_string($table) . '`;');
				}
		}
	}
